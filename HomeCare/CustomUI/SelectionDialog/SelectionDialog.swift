...
open class SelectionDialog: UIView {
    ...
    open func addItem(item itemTitle: String, icon: UIImage,isChecked:Bool, didTapHandler: @escaping (() -> Void)) {
        let item = SelectionDialogItem(item: itemTitle, icon: icon,isChecked:isChecked ,didTapHandler: didTapHandler)
        items.append(item)
    }
    ...
    fileprivate func createContainerView() -> UIScrollView {
        ...
        for (index, item) in items.enumerated() {
            ...
            if item.isSelected{
                let checkImg = UIImageView(frame: CGRect(x: 270, y: 15, width: 20, height: 20))
                checkImg.image = UIImage(named: "ic_check_24")
                itemButton.addSubview(checkImg)
            }
